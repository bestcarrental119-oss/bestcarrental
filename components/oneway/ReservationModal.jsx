...
import { calcAuthorization, ONE_WAY_DEPOSIT, routeRentalBaseAmount, routeRentalDays } from '../../lib/oneWay';
import DamageConsent from '../DamageConsent';

const yen = n => `¥${Number(n || 0).toLocaleString()}`;
const dateLabel = (value) => value ? `${value} 10:00` : '';
...
export default function OneWayReservationModal({ listing, search = {}, onClose, onReserved }) {
  ...
  const [consent, setConsent] = useState({ fuel: false, clean: false, noChange: false, damage: false });
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  const [done, setDone] = useState(null);
  const pickupDate = search.pickupDate ?? '';
  ...
  const days = routeRentalDays(search, fallbackDays);
  const rentalBaseAmount = routeRentalBaseAmount(listing, search, fallbackDays);

  const plan = plans.find(p => p.id === planId) ?? plans[0];
  const insuranceAmount = (plan?.price ?? 0) * days;
  const auth = calcAuthorization({ base: rentalBaseAmount, insurance: insuranceAmount, deposit: ONE_WAY_DEPOSIT });
  const allConsented = consent.fuel && consent.clean && consent.noChange && consent.damage;
    ...
    try {
      const res = await fetch('/api/one-way/reserve', {
        ...
        body: JSON.stringify({
          ...
          insuranceAmount,
          pickupAt: pickupDate ? `${pickupDate}T10:00` : null,
          returnAt: returnDate ? `${returnDate}T10:00` : null,
        }),
      });
        ...
        {done ? (
          ...
          <>
            ...
            {(pickupDate || returnDate) && (
              <div className="mb-4 rounded-xl border border-purple-700/40 bg-purple-900/20 p-3">
                <p className="mb-2 text-xs font-semibold text-purple-100">{t('dateConfirmationTitle')}</p>
                <div className="space-y-1 text-xs">
                  {pickupDate && (
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-gray-400">{t('ow_pickupDate')}</span>
                      <span className="font-bold text-white">{dateLabel(pickupDate)}</span>
                    </div>
                  )}
                  {returnDate && (
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-gray-400">{t('ow_returnDate')}</span>
                      <span className="font-bold text-white">{dateLabel(returnDate)}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="mb-4 flex items-center gap-3 rounded-xl border border-gray-700 bg-gray-800/40 p-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  {t('ow_baseFee')}: <span className="font-bold text-white">{rentalBaseAmount > 0 ? yen(rentalBaseAmount) : t('ow_free')}</span>
                  {days > 1 ? <span className="text-gray-500"> · {days}d</span> : null}
                </p>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="mt-2">
                <DamageConsent checked={consent.damage} onToggle={() => setConsent(c => ({ ...c, damage: !c.damage }))} />
              </div>
            </div>
